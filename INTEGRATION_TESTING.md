...
| **I-30** | Set `connection.type: sqlserver` (or use a `jdbc:sqlserver://host:1433;databaseName=foo` URL). Generate. Run `mvn -f generated/pom.xml compile`. With Docker enabled, `docker compose up db` should start a SQL Server container. | The pom uses `com.microsoft.sqlserver:mssql-jdbc` and Testcontainers `mssqlserver`. `application.yml` has `jdbc:sqlserver://...;databaseName=...` URL + `com.microsoft.sqlserver.jdbc.SQLServerDriver` + `default_schema: dbo`. `docker-compose.yml` uses `mcr.microsoft.com/mssql/server:2022-latest` with `ACCEPT_EULA: Y` and a strong `MSSQL_SA_PASSWORD`. Build compiles cleanly. | ☐ |
| **I-31** | Use `schemaFile:` with a T-SQL `.sql` containing `IDENTITY(1,1)` PK columns + `NVARCHAR(MAX)` + `[bracket]` quoting + `dbo.` schema prefixes. Generate. | Tables, columns, PKs, types parsed correctly. `IDENTITY(1,1)` columns surface as `autoIncrement=true` on the entity. `NVARCHAR(MAX)` resolves to the appropriate Java type via the existing `JavaTypeMapper`. `[bracket]` identifiers stripped to bare names. Generated code compiles. | ☐ |
| **I-32** | Set `connection.type: sqlite` with `database: ./shop.db` (or use a `jdbc:sqlite:./shop.db` URL). Generate with `persistence: jpa`. Run `mvn -f generated/pom.xml compile`. With Docker enabled, `docker compose up app` should start without a separate db service. | The pom uses `org.xerial:sqlite-jdbc` and `org.hibernate.orm:hibernate-community-dialects`. `application.yml` has `jdbc:sqlite:` URL + `org.sqlite.JDBC` driver + explicit `hibernate.dialect: org.hibernate.community.dialect.SQLiteDialect` override + NO `default_schema` line. `docker-compose.yml` has only the app service with a `db-data:/data` volume mount. With `tests.enabled: true`, `AbstractIntegrationTest` uses `@DynamicPropertySource` against `jdbc:sqlite::memory:` with `create-drop` ddl-auto and no `@Testcontainers`. Build compiles cleanly. | ☐ |
| **I-33** | Use `schemaFile: ./schema.sqlite.sql` with a SQLite DDL containing `INTEGER PRIMARY KEY` (no AUTOINCREMENT keyword) + `INTEGER PRIMARY KEY AUTOINCREMENT` + table-level `PRIMARY KEY (id)` for an INTEGER column. Generate with `connection.type: sqlite` (so dialect hint flows through). | All three forms produce `autoIncrement=true` on the entity. `@Generated` strategy on the JPA entity reflects this. Same DDL with `connection.type: postgresql` does NOT mark `autoIncrement` — postgres needs explicit SERIAL. | ☐ |
| **I-34** | Open Settings → Tools → Umaboot. Try each of the three Source radios (Host / URL / Script). | Picking Host: Host/Database/Parameters fields show in the card; Schema/Username/Password rows + Test Connection button are visible. Picking URL: only the JDBC URL field shows in the card; same shared rows below. Picking Script: only the Schema file field + Browse button show in the card; Schema/Username/Password rows + Test Connection button are hidden (they don't apply to file parsing). The Database type combo stays visible across all three modes since it's the dialect hint for both JDBC introspection and JSqlParser. | ☐ |
| **I-35** | In Script mode with a valid `.sql` file picked, click Refresh Tables. | Table list populates from the parsed file (no JDBC connection attempt). Status label shows "N tables parsed from file.sql". Selecting tables and clicking Apply round-trips through `umaboot.yaml` as `schemaFile: ./file.sql` with no `connection:` block. | ☐ |
| **I-36** | Set `generation.buildTool: gradle`. Generate. From `generated/`, run `gradle wrapper` once (creates `gradlew` + `gradle-wrapper.jar`), then `./gradlew bootJar`. | The generated project has `build.gradle.kts` + `settings.gradle.kts` (no `pom.xml`). Build produces a runnable jar under `build/libs/`. With `persistence: jooq`, `./gradlew compileJava` runs the `nu.studer.jooq` plugin's codegen first and produces `${basePackage}.jooq.*` classes under `build/generated/jooq/main`. | ☐ |
| **I-37** | Switch an existing project's `umaboot.yaml` from `buildTool: maven` to `buildTool: gradle`, regenerate into the same output dir, and check what changed. | Generation produces the new gradle files alongside the old `pom.xml` (or replaces it depending on overlay mode). The user's app code (entities, controllers, services) is byte-identical between the two — only the build-config files differ. | ☐ |
